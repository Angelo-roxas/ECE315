...
static void vUartManagerTask( void *pvParameters )
{
	...
	while(1){
		if(flag==1){ 						// flag is set to 1 when the user enters the end sequence on SPI1-SPI0 mode
			...
			// 1. Send a dummy character to uart_to_spi queue to initiate SPI communication.
			xQueueSendToBack(uart_to_spi, &dummy, portMAX_DELAY);
			
			// 2. Wait for SPI data response from spi_to_uart queue.
			//maybe add delay fpr "waiting" or while loop
			xQueueReceive(spi_to_uart, &spi_data, portMAX_DELAY);
			
			// 3. If received character is the dummy character, exit the loop.
			if (spi_data == CHAR_DOLLAR) {
				break;
			}
			
			// 4. Wait if UART transmit buffer is full.
			while (XUartPs_IsTransmitFull(Config->BaseAddress) == TRUE);
			
			// 5. Send received SPI byte to UART.
...
static void vSpiMainTask( void *pvParameters )
{
	...
	while(1){
		...
		if(active_command == 2){
			...
			// 1. Copy received data into send_buffer
			send_buffer[0] = received_from_uart;

			// 2. Increment the received bytes counter
			received_bytes++;

			// 3. Perform SPI transfer
			XSpiPs_PolledTransfer(&SpiInstance, send_buffer, &send_to_uart, TRANSFER_SIZE_IN_BYTES);

			// 4. Send the received byte from SPI to spi_to_uart queue
			xQueueSend(spi_to_uart, &send_to_uart, portMAX_DELAY);
		/*******************************************/
			}
		}
		vTaskDelay(1);
...
static void vSpiSubTask( void *pvParameters )
{
	...
	while(1){
		...
		temp_store = RxBuffer_Slave(); // Read from SPI slave
                if (temp_store == '$') continue; // Ignore dummy bytes

                spi_rx_bytes++;
                msg_bytes++;

                // Check for termination sequence
                if (temp_store == termination_seq[term_index]) {
                    term_index++;
                    if (term_index == 3) { // Sequence detected
                        termination_flag = 3;
                        message_counter++;
                        ...
                        // Send the summary back to the SPI master
                        for (int i = 0; i < str_length; i++) {
                            SpiSlave_Write(buffer[i]);
                        }
                    ...
                    term_index = 0; // Reset if sequence breaks
                }
            }
		}
		vTaskDelay(1);
