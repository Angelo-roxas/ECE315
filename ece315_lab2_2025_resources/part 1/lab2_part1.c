...
void vUserCreateTask(void *pvParameters)
{
    ...
    while(1){
		...
		while (xQueueReceive(xHashResultQueue, &userData, portMAX_DELAY) != pdPASS);
...
void vHashingTask(void *pvParameters)
{
    ...
    while (1){
		...
		if (xQueueReceive(xUserDataQueue, &userData, portMAX_DELAY) == pdPASS){
...
void vUartInputTask(void *pvParameters)
{
	...
	while (1) {
        ...
        vTaskDelay(UART_POLL_PERIOD / portTICK_PERIOD_MS);
		/*****************************************************************************/
	}
}
