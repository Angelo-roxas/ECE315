...
/*************************** Enter your code here ****************************/
	// TODO: receive data from the queue and send it through the UART interface
			if(xQueueReceiveFromISR(xTxQueue, &data, taskToSwitch) == pdTRUE) {
				...
				// Optionally, update any counters here if needed, e.g. countSent++;
			}
/*****************************************************************************/
	    }
	}
...
/*************************** Enter your code here ****************************/
	...
	// Enqueue the data into the TX queue, waiting until there is space
    xQueueSend(xTxQueue, &data, portMAX_DELAY);

    // Enable the TX empty interrupt so that the ISR will pick up
    // the newly queued data and send it out.
    enableTxEmpty();
